...
`make setup` 首先检查 Docker runtime：存在 `nvidia` 时选择 `WhisperLocal / cuda / float16`，否则选择 `FunASR / cpu / int8`。镜像构建后，它还会通过同一个 `xiaozhi-esp32-server` Compose 服务启动一次临时容器，检查 `ctranslate2` 能否发现 CUDA 设备并支持所选的 `ASR_COMPUTE_TYPE`。`auto` 模式下探针失败会回退 CPU；`cuda` 或使用 CUDA 的 `manual` 配置则会终止部署。

最终解析出的 `ASR_MODULE`、`ASR_DEVICE`、`ASR_COMPUTE_TYPE`、`XIAOZHI_CONTAINER_RUNTIME` 和 `NVIDIA_VISIBLE_DEVICES` 会写回 `.env`，因此后续直接运行 `docker compose up` 不会丢失已经验证的选择。
