...
The xiaozhi-server container mounts the same state file read-only. On the live `PiVoiceLLM` path, `pi_voice.py` re-reads it at the start of every voice turn and applies it to both the suffix and output filter. Dashboard changes therefore reach the next turn without restarting containers. If the file is absent or malformed, the provider falls back to `DOTTY_KID_MODE` (default `true`). The dotty-pi RPC wrapper still loads its configured system prompt separately.
...
disables implicit Pi context files. The dotty-pi wrapper supplies an explicit
system prompt, while the suffix re-injects language, format, and Kid Mode policy
on every turn at the end of the user prompt.
