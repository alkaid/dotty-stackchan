...
- **Two-tier voice LLM (Tier1Slim)** -- `qwen3.5:4b` on local llama-swap handles plain conversational turns directly; tool calls (`memory_lookup`, `think_hard`, `take_photo`, `play_song`) escalate to the bridge via `/api/voice/escalate`. Default LLM since commit `b73f583`. See [`tier1slim.md`](https://brettkinny.github.io/dotty-stackchan/latest/tier1slim/)
